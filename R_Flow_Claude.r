...
load_experiment_cached <- function(experiment_path, cache_dir = "flowset_cache") {
  experiment_name <- basename(experiment_path)
  ...
  if (file.exists(cache_file)) {
    fcs_files <- list.files(experiment_path, pattern = "\\.fcs$",
                            full.names = TRUE, ignore.case = TRUE)
    if (length(fcs_files) > 0 && all(file.mtime(fcs_files) < file.mtime(cache_file))) {
      cat(sprintf("  Loading from flowset cache: %s\n", experiment_name))
...
apply_sequential_gates <- function(fcs_data, up_to_gate, gates = GATES, channels = CHANNELS) {
  ...
  if(up_to_gate >= 3) {
    if(is.matrix(gates$live_cells)) {
      ...
      # New threshold-based gate
      dcm_values <- exprs(fcs_data)[, channels$DCM]
      live_filter <- dcm_values < gates$live_cells$threshold
    }
